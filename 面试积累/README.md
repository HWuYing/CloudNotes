...
2、有没有看过react经过webpack打包之后的JS文件
        （dependencies 上线打包需要打包的内容模块    devdependencies  开发需要依赖的包，一般webpack放这儿）
        webpack打包代码，是基于模块来打包的，也就是说，经过打包的代码是被放在了一个函数里面，这时候所有的变量和方法都已经变成了局部的, 并且这是一个立即执行函数，会创建独立的作用域。这个立即执行函数会接收一个参数作为数组modules，数组的每一项都是一个匿名函数代表一个模块。    
        Web pack 经过分析之后，会得到相应的语法树，通过递归检测到当前模块所依赖的所有模块，以及依赖的依赖，整合之后，最终合并到代码中去。

3、浅拷贝 与 深拷贝
        浅拷贝： Object.assign( )、… 结构语法
            这两种拷贝，基本上只对第一层有作用，超过一层之后的数据，一处修改，另外一处也会修改到
            

        深拷贝：JSON.parse( JSON.stringify( obj ) )
        此时的深拷贝，函数不会被拷贝成功； 以及原型链上的东西，不会被拷贝成功。会忽略undefined，也不能序列化函数，不能解决循环引用的对象。
